...
export interface IStorage {
  ...
  updateCollege(id: string, updates: Partial<CollegeType>): Promise<CollegeType | null>;
  ...
  updateEvent(id: string, updates: Partial<EventType>): Promise<EventType | null>;
  ...
  updateDonation(id: string, updates: Partial<DonationType>): Promise<DonationType | null>;
  ...
  updateConnection(id: string, updates: Partial<ConnectionType>): Promise<ConnectionType | null>;
...
export class MongoStorage implements IStorage {
  ...
  private async initializeConnection() {
    try {
      const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/oualumnihub';
        ...
        for (const [key, value] of Object.entries(obj)) {
          if (value && typeof value === 'object' && value.toString && value.constructor && value.constructor.name === 'ObjectId') {
            ...
            converted[key] = value.map(item => 
              item && typeof item === 'object' && item.toString && item.constructor && item.constructor.name === 'ObjectId' 
                ? item.toString() 
  ...
  async searchUsers(query: string, filters: any = {}): Promise<UserType[]> {
    try {
      ...
      return users.map(user => this.convertDocToType<UserType>(user)).filter(Boolean) as UserType[];
    } catch (error) {
  ...
  async getColleges(filters: any = {}): Promise<CollegeType[]> {
    try {
      const query = filters.type ? { type: filters.type, isActive: true } : { isActive: true };
  ...
  async updateCollege(id: string, updates: Partial<CollegeType>): Promise<CollegeType | null> {
    try {
      const updated = await College.findByIdAndUpdate(id, updates, { new: true }).lean();
  ...
  async getEvent(id: string): Promise<EventType | null> {
    try {
      const event = await Event.findById(id).populate('organizer', 'firstName lastName username').populate('college', 'name type').lean();
      return event as any;
    } catch (error) {
  ...
  async updateEvent(id: string, updates: Partial<EventType>): Promise<EventType | null> {
    try {
  ...
  async getDonation(id: string): Promise<DonationType | null> {
    try {
      const donation = await Donation.findById(id).populate('donor', 'firstName lastName username').lean();
  ...
  async updateDonation(id: string, updates: Partial<DonationType>): Promise<DonationType | null> {
    try {
      const updated = await Donation.findByIdAndUpdate(id, updates, { new: true }).lean();
      return updated as any;
    } catch (error) {
  ...
  async getChat(id: string): Promise<ChatType | null> {
    try {
      const chat = await Chat.findById(id).populate('participants', 'firstName lastName username profile.avatar').lean();
      return chat as any;
    } catch (error) {
  ...
  async updateChat(id: string, updates: Partial<ChatType>): Promise<ChatType | null> {
    try {
      const updated = await Chat.findByIdAndUpdate(id, updates, { new: true }).lean();
      return updated as any;
    } catch (error) {
  ...
  async updatePost(id: string, updates: Partial<PostType>): Promise<PostType | null> {
    try {
      const updated = await Post.findByIdAndUpdate(id, updates, { new: true }).lean();
      return updated as any;
    } catch (error) {
  ...
  async createConnection(connection: InsertConnection): Promise<ConnectionType> {
    try {
  ...
  async updateConnection(id: string, updates: Partial<ConnectionType>): Promise<ConnectionType | null> {
    try {
      const updated = await Connection.findByIdAndUpdate(id, updates, { new: true }).lean();
      return updated as any;
    } catch (error) {
  ...
  async getNetworkStats(): Promise<any> {
    try {
      const [totalUsers, totalColleges, totalEvents, totalDonations] = await Promise.all([
        User.countDocuments(),
